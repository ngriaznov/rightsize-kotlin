...
class MsbProvisionerTest {
    ...
    // --- Version pin: MSB_VERSION applies to every platform.

    @Test fun `install dir is named after the pinned version`() {
        ...
        assertEquals(MsbProvisioner.MSB_VERSION, msbWin.parent.parent.fileName.toString())

        val msbUnix = MsbProvisioner.ensureInstalled(baseUrl, cache, emptyMap(), Platform.LINUX_X64)
        assertEquals(MsbProvisioner.MSB_VERSION, msbUnix.parent.parent.fileName.toString())
    }
}
