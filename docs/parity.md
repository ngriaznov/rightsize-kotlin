...
| File copy-in | `withCopyFileToContainer` round-trips a classpath resource and a host-path file into the guest at the given path; the default read-only mount rejects an in-guest write (where the backend enforces it — see [Backends](backends.md#backend-differences)). |
...
| Capabilities | `capabilities.hardwareIsolated` and `capabilities.checkpoint` report each backend's actual, fixed values (microsandbox: isolated, no checkpoint; Docker: not isolated, checkpoint via image commit) — never a guess, never negotiated at runtime. |
...
| Checkpoint gating | `checkpoint()` succeeds only on a backend whose `capabilities.checkpoint` is `true`, and throws a typed, backend-naming error before any backend call otherwise. |
