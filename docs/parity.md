...
| UDP host port mapping | `withExposedUdpPorts` publishes a guest port over UDP at a real, reachable host port, resolvable via `getMappedUdpPort` — a host UDP datagram reaches the guest on both backends. The same guest port number can be exposed on both protocols at once (e.g. DNS's 53) without either mapping colliding. Container-to-container UDP over a `Network` is Docker-only in this phase — see [Networking](concepts/networking.md#udp). |
...
| File copy-in | `withCopyFileToContainer` round-trips a classpath resource and a host-path file into the guest at the given path; the default mount is read-write and is a view of the host file (an in-guest write reaches it), while a mount created with `readOnly = true` rejects an in-guest write on both backends. This is a start-time mount, configured before `start()` — see [Copying Files](copy.md) for the runtime equivalent below. |
